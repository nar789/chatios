...
struct BoardRowView {
    ...
    var mainClub_MainPage_Top10: MainClub_Popular10_PostList?
    
    var mainHome_home: MainPostDtoList?
    ...
    //var mainClub_ChallengePage: MainClub_MainPage_PopularTop10_tmp?
    var homeClub_TabHomeModel_BoardList: HomeClub_TabHomeModel_BoardList?
    var homeClub_TabHome_PostItem: HomeClub_TabHome_Post_Model?
    var homeClub_TabFreeboardItem: HomeClub_TabFreeboardModel_FreeBoard?
    var homeClub_TabFreeboard_PostItem: HomeClub_Board_Post_Model?
    var homeClub_Notice_Item: HomeClub_Board_Post_Model?
    ...
    @State var storagelikeCount: Int = 0
    @State var attachArr: AttachList?
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Group {
                ...
                else if viewType == .MainHome_Home {
                    header_type03
                        .padding(.horizontal, sizeInfo.Hpadding)
                }
                ...
                else if viewType == .HomeClub_Notice {
                    header_type03
                        .padding(.horizontal, sizeInfo.Hpadding)
                }
            }
            ...
            Group {
                if viewType == .HomeClubType1 {
                    ...
                    if (video?.video_url ?? "") != "" {
                        ...
                        if (imageArr?.count ?? 0) > 0 {
                            body_type04
                        }
                        else if (webLink?.link_url ?? "") != "" {
                            body_type06
                        }
                        else {
                            body_type07
...
//                }
                ...
                else if (viewType == .MainCommunity_Hour) || (viewType == .MainCommunity_Week) || (viewType == .HomeCommunity_List_YesHot) {
                    body_type02
                ...
                else if (viewType == .MainCommunityMyLocalBoard) || (viewType == .MainClub_MainPage_Top10) {
                    body_type02
                        .padding(EdgeInsets(top: 10, leading: sizeInfo.Hpadding, bottom: 10, trailing: sizeInfo.Hpadding))
                }
                else if viewType == .HomeCommunity_List_NoHot {
    ...
    var header_type02: some View {
        HStack(spacing: 0) {
            ...
            if
                viewType != .HomeClubType1,
                viewType != .MainCommunity_Hour,
                viewType != .MainCommunity_Week,
                viewType != .MyClub_Storage_Post,
                viewType != .MyClub_Storage_Comment,
                viewType != .MyClub_Storage_Bookmark,
                viewType != .MyClub_Member_Storage_Post,
                viewType != .MyClub_Member_Storage_Reply,
                viewType != .MyCommunity_Storage_Comment,
                viewType != .MyCommunity_Storage_Post,
                viewType != .MyCommunity_Storage_BookMark,
                viewType != .Community_Notice_Total,
                viewType != .HomeCommunity_List_YesHot,
                viewType != .HomeCommunity_List_NoHot
            {
                Image("icon_outline_more")
                    .resizable()
                    .frame(width: 24, height: 24)
    ...
    var header_type03: some View {
        HStack(spacing: 0) {
            ...
            if
                ...
                viewType != .MyClub_Storage_Comment,
                viewType != .MyClub_Storage_Bookmark,
                viewType != .MainClub_MainPage_Top10,
                viewType != .HomeClub_Notice
            {
                Button(action: {
            ...
            if let NOisBlockedBoard = isBlockedBoard {
                if NOisBlockedBoard {
                    ...
                    if viewType == .Community_Notice_Total {
                         ...
                         */
                        (
                            // 아이콘 높이는 텍스트 높이보다 약간 높기 때문에, 2.5만큼 아래로 낮춰줘서 텍스트와 중앙을 맞춰준다.
                            Text(Image(self.setTitleImage())).baselineOffset(-2.5)
                            +
                            Text(" \(title)").font(.body21420Regular).foregroundColor(.gray870)
                        )
                    }
    ...
    var body_type03: some View {
         ...
         */
        (
            Text("[\(beginningOfTitle ?? "")] ").font(.buttons1420Medium).foregroundColor(.primary500)
            +
            // 아이콘 높이는 텍스트 높이보다 약간 높기 때문에, 2.5만큼 아래로 낮춰줘서 텍스트와 중앙을 맞춰준다.
            Text(Image(self.setTitleImage())).baselineOffset(-2.5)
            +
    ...
    func setData() {
        ...
        if viewType == .HomeClubType1 {
            ...
            if let NOboardItem = clubStoragePostListData {
                authorImage = NOboardItem.profileImg ?? ""
                boardName = NOboardItem.categoryName1 ?? ""
                authorNickname = NOboardItem.nickname ?? ""
                boardDate = NOboardItem.createDate ?? ""
                title = NOboardItem.subject ?? ""
                //                imageArr = NOboardItem.post_thumbnail
            ...
            if let NOboardItem = communityStoragePostListData {
                ...
                storagelikeCount = NOboardItem.likeCnt
                //                imageArr = NOboardItem.post_thumbnail
                //                video = NOboardItem.post_video
            ...
            if let NOboardItem = mainHome_home {
                if NOboardItem.type == "COM" {
                    authorImage = NOboardItem.comPost?.userPhoto ?? ""
                    boardName = NOboardItem.comPost?.code ?? ""
                    authorNickname = NOboardItem.comPost?.userNick ?? ""
                    boardDate = NOboardItem.comPost?.createDate ?? ""
                    title = NOboardItem.comPost?.title ?? ""
                    bodyTxt = NOboardItem.comPost?.content ?? ""
//                    image = NOboardItem.comPost?.categoryImage.imageOriginalUrl ?? ""
                    image = "d66455fc-704b-4ac2-9ac8-789eae683900".imageOriginalUrl
                    //                video = NOboardItem.
                    //                webLink = NOboardItem.
                    commentCount = NOboardItem.comPost?.replyCnt ?? 0
            }
            else {
                authorImage = NOboardItem.clubPost?.profileImg ?? ""
                boardName = NOboardItem.clubPost?.subject ?? ""
                authorNickname = NOboardItem.clubPost?.nickname ?? ""
                boardDate = NOboardItem.clubPost?.createDate ?? ""
                title = NOboardItem.clubPost?.subject ?? ""
                bodyTxt = NOboardItem.clubPost?.content ?? ""
                image = NOboardItem.clubPost?.firstImage.imageOriginalUrl ?? ""
                    //                video = NOboardItem.firstImage
                    //                webLink = NOboardItem.link
                commentCount = NOboardItem.clubPost?.replyCount ?? 0
                }
            }
        }
        ...
        // 2.0 적용하면 지울 것
            ...
            if let item = homeClub_TabHome_PostItem {
                authorImage = item.profileImg ?? ""
                boardName = item.categoryName2 ?? ""
                authorNickname = item.nickname ?? ""
                boardDate = item.createDate ?? ""
                title = item.subject ?? ""
                bodyTxt = item.content ?? ""
                imageArr = item.attachList?.filter { $0.attachType == 0 }.map { $0.toPostThumbnail() }
                //                video = item.post_video
            ...
            if let item = homeClub_TabFreeboard_PostItem {
                authorImage = item.profileImg ?? ""
                boardName = item.categoryName2 ?? ""
                authorNickname = item.nickname ?? ""
                boardDate = item.createDate ?? ""
            ...
            if let item = homeClub_Notice_Item {
                authorImage = item.profileImg ?? ""
                boardName = item.categoryName2 ?? ""
                authorNickname = item.nickname ?? ""
                boardDate = item.createDate ?? ""
    ...
    func setTitleImage() -> String {
        ...
        if viewType == .Community_Notice_Total {
            result = "posting_fix"
                    ...
                    (viewType == .HomeCommunity_List_NoHot) {
            result = attachYn ?? false ? "posting_file" : "posting_text"
        }
        else {
            /**
    ...
    func bodyClickNavigation() -> some View {
        NavigationLink("", isActive: $showDetail) {
            ...
            if viewType == .MainPopular {
                ...
                if let NOboardItem = mainCommunity_BoardItem {
                    CommunityDetailPage(viewType: viewType, selectedCode: NOboardItem.code, selectedPostId: NOboardItem.postId)
                }
            }
            else if viewType == .HomeClub_TabFreeboard {
                if let NOboardItem = homeClub_TabFreeboardItem {
            ...
            else if viewType == .HomeClubType1 {
                if let NOboardItem = fantooTVHomeTabItem {
                    ClubDetailPage()
                }
            }
                ...
                if let NOboardItem = mainClub_MainPage_Top10 {
                    ClubDetailPage(postId: NOboardItem.postId,
