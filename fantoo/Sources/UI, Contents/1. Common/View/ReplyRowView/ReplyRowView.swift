...
struct ReplyRowView {
    ...
    let viewType: ReplyType
    var replyData: CommonReplyModel?
    var replyDetailData: CommonReplyModel?
    var onPress: ((ReplyButtonType) -> Void)? // BoardRowView를 호출하는 부분 전부 다 수정되면 옵셔널 제거할 것!
...
extension ReplyRowView {
    ...
    func bodyClickNavigation() -> some View {
        NavigationLink("", isActive: $showDetail) {
            if viewType == .CommunityDetail {
                if let NOboardItem = replyData {
                    ReplyDetailPage(postId: NOboardItem.comPostId, replyId: NOboardItem.replyId)
                }
            }
        }.hidden()
    }
