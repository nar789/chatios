...
struct FindPasswordPage: View {
    ...
    var body: some View {
        VStack(alignment: .center, spacing: 0) {
            ...
            if emailCorrectStatus == .Correct {
                Button {
                    vm.tempPassword(loginId: email) { success in
                        if success {
                            userManager.certNumberAlert = true
                        }
                    }
                } label: {
    ...
    // 인증번호 전송 팝업
        .popup(isPresenting: $userManager.certNumberAlert, cornerRadius: 5, locationType: .bottom, autoDismiss: .after(2), popup:
                ZStack {
            Spacer()
