            ...
            VStack {
                ScrollView {
                    LazyVStack(spacing: 0) {
                        ...
                        if vm.openYn {
                            ClubManagementProfileView(clubName: vm.myClubName, openDate: vm.clubOpenDate, open: "g_open_public".localized, clubImageType: .Public) {
                                buttonType in
                            ...
                            ClubManagementProfileView(clubName: vm.myClubName, openDate: vm.clubOpenDate, open: "b_hidden".localized, clubImageType: .Private) {
                                buttonType in
                                print("buttonType : \(buttonType)")
                                if buttonType == .EditProfile {
                                }
                        ...
                        .background(
                            NavigationLink("", isActive: $showStandByJoinClubMember) {
                                StandByJoinClubMemberPage(clubId: $clubId)
                            }.hidden()
                        )
                            ...
                            NavigationLink("", isActive: $showInfoSetting) {
                                if vm.clubMemberCount == 1 {
                                    if vm.closesStatus == 0 {
                                        ClubClosingPage(clubClosingType: .WriteClubClosing)
                                    }
                                    else if vm.closesStatus == 1 {
                                        ClubClosingPage(clubClosingType: .ConfirmClubClosing)
                                    }
                                }
                                else {
                                    if vm.delegateStatus == 0 {
                                        ...
                                        ClubClosingStatePage(clubId: $clubId, delegateDate: "멤버 수락 후 확정", memberNickname: "사용자 닉네임", memberId: "abcdefg", memberLevel: "a_general_membership".localized)
                                    }
                                }
                            }.hidden()
                        )
