...
struct StandByJoinClubMemberPage_S {
    ...
    @State var approvalAllJoinIdList: [Int] = []
...
extension StandByJoinClubMemberPage_S: View {
        ...
        ZStack {
            VStack {
                ...
                ZStack {
                    HStack {
                        Button {
                            vm.isSelectedAll.toggle()
                        } label: {
                            Image(vm.isSelectedAll ? "Checkbox_login_checked" : "Checkbox_login_unchecked")
            ...
            .onChange(of: vm.isSelectedAll) { _ in
                vm.changeAllListIsSelected(isSelectedAll: vm.isSelectedAll)
            }
    ...
    var listView: some View {
        ForEach(Array(vm.clubJoinListData.enumerated()), id: \.offset) { index, element in
            ZStack {
                let _ = print("element : \(element)")
                HStack(spacing: 0) {
                    Button {
                        vm.selectedListItem(index: index)
                    } label: {
                        Image(element.isSelected ? "Checkbox_login_checked" : "Checkbox_login_unchecked")
                            .frame(width:sizeInfo.checkSize.width, height:sizeInfo.checkSize.height)
                            .padding(.trailing, sizeInfo.padding12)
                    }
...
struct StandByJoinClubMemberPage_S_RowView: View {
    ...
    var viewModel = StandByJoinClubMemberViewModel()
    let showLine: Bool
