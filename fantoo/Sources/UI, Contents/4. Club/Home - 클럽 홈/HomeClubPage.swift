...
struct HomeClubPage {
    ...
    @State private var isPresentedJoinPage = false
    ...
    @State private var showNeedJoinToast = false
    @State private var showJoinWaitingPopup = false
    @State private var showingClubInfoPopup = false
...
//                            .tag(3)
            ...
            Button(action: {
                if let login = viewModel_home.homeClub_Login {
                    if login.getJoinStatus() == .Momber {
            ...
            .padding(.bottom, needsJoinUser() ? 98 + 100 : 52 + 100)   // ScrollView 길이를 100 추가했으니, padding bottom 값도 100 추가
            
            if needsJoinUser() {
                CommonButton2(
                    title: viewModel_home.homeClub_Login?.getJoinStatus() == .RequsetJoin ? "g_to_wait_join".localized : "g_to_join".localized,
                    type: .defaults(),
                    disabled: $buttonDisabled
                )
                .padding(EdgeInsets(top: 0, leading: 20, bottom: 24 + 100, trailing: 20))
                .onTapGesture {
                    switch viewModel_home.homeClub_Login?.getJoinStatus() ?? .NoMember {
                    case .RequsetJoin:
                        showJoinWaitingPopup.toggle()
                    default:
                        isPresentedJoinPage.toggle()
                    }
                }
            }
...
//                    state = false
        ...
        .fullScreenCover(isPresented: $isPresentedJoinPage) {
            ClubJoinPage(
                clubId: clubId,
                clubName: viewModel_home.homeClub_BasicInfo?.clubName ?? "") { joinAutoYn in
                    if let yn = joinAutoYn {
                        if yn {
                            callRemoteData()
                        } else {
                            viewModel_home.clubLogin(clubId)
                        }
                    }
            }
        }
        .popup(
            isPresented: $showNeedJoinToast,
            type: .floater(),
            position: .bottom,
            animation: .spring(),
            autohideIn: 2,
            ...
            view: {
                ToastView(text: "클럽 가입 후 작성하실 수 있습니다.")
            }
        )
        .popup(
            isPresented: $showingClubInfoPopup,
            type: .`default`,
            closeOnTap: false,
            backgroundColor: .black.opacity(0.4),
            view: {
                if let info = $viewModel_home.homeClub_BasicInfo {
                    ClubInfoPopupView(isPresented: $showingClubInfoPopup,
                                      model: info)
                }
            }
        )
        .popup(isPresented: $showJoinWaitingPopup,
               type: .`default`,
        ...
        .popup(
            isPresented: $viewModel_home.showClubDelegatePopup,
            type: .`default`,
            animation: Animation.easeOut(duration: 0.15),
            closeOnTap: false,
            backgroundColor: .black.opacity(0.4),
            view: {
                if let model = viewModel_home.homeClub_Delegate {
                    HomeClubDelegatePopupView(isPresented: $viewModel_home.showClubDelegatePopup,
                                              clubId: clubId,
                                              clubName: viewModel_home.homeClub_BasicInfo?.clubName ?? "",
                                              model: model,
                                              buttonHandler: { action in
                        switch action {
                        case .Accept:
                            print("")
                        case .Refuse:
    ...
    private var header: some View {
        ZStack {
            ...
            Group {
                WebImage(url: URL(string: viewModel_home.homeClub_BasicInfo?.bgImg?.imageOriginalUrl ?? ""))
                    .placeholder(content: {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ZStack {
                    VStack(spacing: 0) {
                        ...
                        Group {
                            HStack(spacing: 0) {
                                WebImage(url: URL(string: viewModel_home.homeClub_BasicInfo?.profileImg?.imageOriginalUrl ?? ""))
                                    .placeholder(content: {
                                ...
                                VStack(spacing: 0) {
                                    HStack(spacing: 0) {
                                        Text(viewModel_home.homeClub_BasicInfo?.clubName ?? "")
                                    ...
                                    HStack(spacing: 0) {
                                        Text("멤버 \(viewModel_home.homeClub_BasicInfo?.memberCount ?? 0)")
                                    ...
                                    .onTapGesture {
                                        self.showingClubInfoPopup = true
                                    }
                                }
                                .padding(.leading, 12)
                            }
                            ...
                            HStack(spacing: 0) {
                                Text(viewModel_home.homeClub_BasicInfo?.introduction ?? "")
                                    .font(.caption11218Regular)
                                    .foregroundColor(.gray25)
                                    .padding(.top, 11)
                                    .padding(.bottom, 14)
...
extension HomeClubPage {
    func callRemoteData() {
        self.viewModel_home.clubLogin(clubId)
        self.viewModel_home.clubVisit(clubId)
        self.viewModel_home.getBasicInfo(clubId)
        self.viewModel_freeboard.getCategory(clubId: clubId)
        self.viewModel_archive.getArchive(clubId: clubId)
    }
    
    func needsJoinUser() -> Bool {
        if let login = viewModel_home.homeClub_Login {
            return login.getJoinStatus() == .NoMember || login.getJoinStatus() == .RequsetJoin || login.getJoinStatus() == .CannotJoin
        }
        return true
    }
}
...
extension HomeClubPage {
    var alertJoinWait: some View {
        VStack(alignment: .center, spacing: 0) {
            ...
            HStack {
                CommonButton2(title: "아니요", type: .defaults(), disabled: .constant(true))
                    .padding(.trailing, 2)
                    .onTapGesture {
                        showJoinWaitingPopup.toggle()
                    }
                
                CommonButton(title: "기입 취소", bgColor: Color.stateEnablePrimaryDefault)
                    .padding(.leading, 2)
                    .onTapGesture {
                        viewModel_home.clubJoinCancel(clubId: clubId)
                        showJoinWaitingPopup.toggle()
                    }
            }
            .padding(.top, 24)
