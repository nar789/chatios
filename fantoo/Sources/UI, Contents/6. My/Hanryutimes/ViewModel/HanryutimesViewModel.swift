        ...
        ApiControl.getHomeClubType1TabInfo(clubId: clubId, integUid: integUid)
            .sink { error in
                ...
                if NOhanryuTimes_TabInfoModel.categoryList.count > 0 {
                    ...
                    let categoryId: Int = NOhanryuTimes_TabInfoModel.categoryList[arrLastIndex].categoryId
                    self.getCategoryTab(clubId: "hanryu_times", integUid: UserManager.shared.uid, categoryCode: String(categoryId))
                }
            }
            .store(in: &canclelables)
    }
