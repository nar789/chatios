...
struct HanryutimesPage {
    @StateObject var viewModel = HanryutimesViewModel()
...
extension HanryutimesPage: View {
    var body: some View {
        ZStack(alignment: .bottomTrailing) {
            if self.viewModel.hanryuTimes_Tabs.count > 0 {
                ScrollView {
                    VStack(spacing: 0) {
        ...
        .showFollowAlert(isPresented: $showInfoAlert, title: "h_hanryutimes".localized, message: "12.345만명", number: "", detailMessage: "세계적인 아이돌 그룹인 BTS와 함께할 클럽원\n모집합니다! 클럽 많은 관심과 사랑 부탁!^^\n먹긴 우리의 BTS팬클럽 소개를 해드릴까해요!!\n클럽 많은 관심과 사랑 부탁드립니다!", buttons: ["공유하기", "d_close".localized], onClick: { buttonIndex in
    ...
    private var tabBar: some View {
        GeometryReader { geometry in
            HomeClubPageTabs(tabs: self.viewModel.hanryuTimes_Tabs, geoWidth: geometry.size.width, selectedTab: $selectedTab)
        }
    }
    ...
    private var channelTabContentsView: some View {
        VStack(spacing: 0) {
            if self.viewModel.newsTab_CategoryList.count > 0 {
                ForEach(Array(self.viewModel.newsTab_CategoryList.enumerated()), id: \.offset) { index, element in
...
extension HanryutimesPage {
    func callRemoteData() {
        self.viewModel.getDetailTop(clubId: "hanryu_times")
        self.viewModel.getTabInfo(clubId: "hanryu_times", integUid: UserManager.shared.uid)
        self.viewModel.getHomeTab(clubId: "hanryu_times", categoryCode: "home", integUid: UserManager.shared.uid, nextId: nil, size: nil)
        self.viewModel.getFollowInfo(clubId: "hanryu_times", integUid: UserManager.shared.uid, access_token: UserManager.shared.accessToken)
    }
    
    func clickLike(categoryCode: String, likeType: String, url_postId: Int, index: Int) {
        self.viewModel.patchHanryuTimesLike(clubId: "hanryu_times", likeType: likeType, categoryCode: categoryCode, url_postId: url_postId, integUid: UserManager.shared.uid, access_token: UserManager.shared.accessToken, clickedIndex: index)
    }
}
