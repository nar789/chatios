        ...
        ApiControl.getHomeClubType1TabInfo(clubId: clubId, integUid: integUid)
            .sink { error in
                ...
                if NOfantooTV_TabInfoModel.categoryList.count > 0 {
                    ...
                    let categoryId: Int = NOfantooTV_TabInfoModel.categoryList[arrLastIndex].categoryId
                    
                    self.getCategoryTab(clubId: "fantoo_tv", integUid: UserManager.shared.uid, categoryCode: String(categoryId))
                }
            }
            .store(in: &canclelables)
    }
