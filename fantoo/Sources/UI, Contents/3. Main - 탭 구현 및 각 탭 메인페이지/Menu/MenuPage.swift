            ...
            ScrollView() {
                LazyVStack(spacing: 0) {
                    ...
                    /* -------------------------------------------------------------------------
                     프로필 완성하기
                     ------------------------------------------------------------------------- */
                    if vm.stepValue < completeStepValue, userManager.isLogin {
                        MenuMakeProfileView(stepValue: vm.stepValue) {
                            showProfilePage = true
                        }
                        .modifier(CornerRadiusListModifier())
                        .background(
                            NavigationLink("", isActive: $showProfilePage) {
                                EditProfilePage(profileUrl: $vm.profileUrl, nickName: $vm.nickName, interests: $vm.interests, countryCode: $vm.countryCode, gender: $vm.gender, birthDay: $vm.birthDay)
                    ...
                    if !userManager.isLogin {
                        Text("MY".localized)
                            ...
                            .padding(EdgeInsets(top: 0, leading: DefineSize.Contents.HorizontalPadding + 10, bottom: sizeInfo.listSpacing, trailing: 0))
                    }
                    ...
                    VStack(spacing: 0) {
                        ...
                        .background(
                            NavigationLink("", isActive: $showInviteFriendPage) {
                                InviteFriendPage(referralCode: $vm.referralCode, useReferralCode: $vm.useReferralCode, checkPassed12Hours: vm.joinedDate.checkOverTime(minutes: 12 * 60))
                            }.hidden()
                        )
                    }
                    .modifier(CornerRadiusListModifier())
