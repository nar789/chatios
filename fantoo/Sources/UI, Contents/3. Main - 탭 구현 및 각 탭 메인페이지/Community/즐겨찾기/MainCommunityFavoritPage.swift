...
struct MainCommunityFavoritPage {
    ...
    // popup
    @State var showSheetFavoriteVisibility = false
...
struct MainCommunity_FavoritPage_Row: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            let urlThumbnailImage: String = DefineUrl.ApiImageDomain + element.categoryImage + urlImageInfo.thumbnailPath
            
            WebImage(url: URL(string: urlThumbnailImage))
                .resizable()
                .placeholder(Image("profile_club_character"))
