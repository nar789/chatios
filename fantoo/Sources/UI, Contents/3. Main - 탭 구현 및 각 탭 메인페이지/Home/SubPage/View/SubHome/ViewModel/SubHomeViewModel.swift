...
class SubHomeViewModel: ObservableObject {
    ...
    @Published var mainHome: MainHomeData?
    @Published var mainPostDtoList = [MainPostDtoList]()
    @Published var listSizeCount: Int = 0
    @Published var comPost: ComPost?
    @Published var clubPost: ClubPost?
    ...
    func requestMainHome() {
        loadingStatus = .ShowWithTouchable
        
        ApiControl.getMainHome_Home(integUid: UserManager.shared.uid, nextId: "", size: 10)
            .sink { error in
                self.loadingStatus = .Close
                guard case let .failure(error) = error else { return }
                
                print("MainHome error : \(error)")
            } receiveValue: { value in
                self.loadingStatus = .Close
                self.mainHome = value
                self.listSizeCount = value.listSize
                
                guard let noMainHome = self.mainHome else {
                    return
                }
                
                self.mainPostDtoList = noMainHome.mainPostDtoList
                print("MainHome value : \(value)")
            }
            .store(in: &canclelables)
    }
