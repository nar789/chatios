...
enum ApisCommunity {
    ...
    case EachCategoryBoards(code: String, globalYn: Bool, integUid: String, nextId: Int, size: Int, subCode: String, access_token: String, currentPopupType: GlobalLanType, selectedLanCode: String)
...
extension ApisCommunity: Moya.TargetType {
    ...
    var path: String {
        ...
        case .EachCategoryBoards(let code, _, _, _, _, _, _, _, _):
    ...
    var task: Task {
        ...
        case .EachCategoryBoards(_, let globalYn, let integUid, let nextId, let size, let subCode, _, _, _):
            var params = defultParams
    ...
    var headers: [String : String]? {
        ...
        case .MemberSearch(_, _, _, _, let access_token):
            ...
            header["access_token"] = access_token
            return header
        case .EachCategoryBoards(_, _, _, _, _, _, let access_token, let currentPopupType, let selectedLanCode):
            
            if currentPopupType == .AnotherLan && selectedLanCode != "" {
                header = CommonFunction.defaultHeader(acceptLanguage: selectedLanCode)
            }
