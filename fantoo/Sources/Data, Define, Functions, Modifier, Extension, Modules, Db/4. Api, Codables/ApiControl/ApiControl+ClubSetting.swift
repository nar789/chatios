...
extension ApiControl {
    ...
    static func myClubMemberBookmarkList(clubId: String, integUid: String, memberId: String, nextId: String, size: String) -> AnyPublisher<ClubStorageBookmarkData, ErrorModel> {
        Future<ClubStorageBookmarkData, ErrorModel> { promise in
            
            let apis: ApisClubSetting = .ClubMemberBookmarkList(clubId: clubId, integUid: integUid, memberId: memberId, nextId: nextId, size: size)
            Check().checkToken(isCheck: apis.isCheckToken()) { success in
                if !success { return }
