...
//                        }
    ...
    // 각 카테고리 게시글 목록
    static func getEachCategoryBoards(code: String, globalYn: Bool, integUid: String, nextId: Int, size: Int, subCode: String, access_token: String, currentPopupType: GlobalLanType, selectedLanCode: String) -> AnyPublisher<Community_List, ErrorModel> {
        Future<Community_List, ErrorModel> { promise in
            
            let apis: ApisCommunity =
                .EachCategoryBoards(code: code, globalYn: globalYn, integUid: integUid, nextId: nextId, size: size, subCode: subCode, access_token: access_token, currentPopupType: currentPopupType, selectedLanCode: selectedLanCode)
