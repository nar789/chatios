//
//  UserManagerKeyChain.swift
//  fantoo
//
//  Created by fns on 2022/06/29.
...
class UserManagerKeyChain: ObservableObject {
    static let shared = UserManagerKeyChain()
    
    func toSHA256(_ password: String) -> String {
        let data = password.data(using: .utf8)
        let sha256 = SHA256.hash(data: data!)
        let shaData = sha256.compactMap{_ in String(format: "%02x, $0")}.joined()
        return shaData
    }
